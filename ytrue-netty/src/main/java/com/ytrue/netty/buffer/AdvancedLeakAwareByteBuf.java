...
/**
 * @Author: ytrue
 * @Description:这个类的每个方法几乎都会调用一下ResourceLeakTracker的record方法，记录自己的调用轨迹 ResourceLeakTracker对象在父类中，是父类的一个成员变量
 * 并且还会打印一下泄漏信息。这个类对应的内存泄漏检测等级为PARANOID
 * 该类中几乎每个方法都会在内部调用一下recordLeakNonRefCountingOperation静态方法，
 * 而该静态方法就是用来记录所包装的PooledDirectByteBuf的调用轨迹的
 */
final class AdvancedLeakAwareByteBuf extends SimpleLeakAwareByteBuf {
    ...
    /**
     * @Author: ytrue
     * @Description:被包装的bytebuf在释放的时候会调用这个方法，这个方法会调用到父类的release方法 在父类的release方法内，会将弱引用对象中的引用清除干净。
     */
    @Override
